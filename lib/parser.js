...
function systemPrompt(state) {
  const dayList = state.days.map((d) => `${d.weekday} ${d.date}`).join(", ");
...
- {"type":"set_required_staff","date":"7/20","count":3}
...
- Weekday shorthand: she may write things like "MWF", "M/W/F", "T/Th", "TuTh", "weekdays". Expand these to every matching date in this week's list and emit one action per date. M=Monday, T or Tu=Tuesday, W=Wednesday, Th=Thursday, F=Friday, Sa=Saturday, Su=Sunday.
- If she gives a headcount directly ("I need 3 workers", "need 2 people"), use set_required_staff — do NOT use set_kids for this, and do not invent a kid count.
- If she gives a kid count instead ("12 kids"), use set_kids as before; the bot will derive the staffing minimum from that unless she later overrides it directly with set_required_staff.
...
- Never invent kid counts, times, staffing counts, or rates that weren't stated.`;
}
